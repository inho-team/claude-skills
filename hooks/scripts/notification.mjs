...
const chains = [
  ...
  { match: 'codex-rescue',       hint: '[CODEX] Codex rescue returned Done. WARNING: Done does NOT mean files are written — Codex companion runs async. You MUST now execute this polling loop: (1) Run `git diff --stat` immediately. (2) If changes found, proceed to verify. (3) If NO changes, poll with `sleep 30 && git diff --stat` up to 10 times (5 min). (4) On each poll tell user: "Polling Codex... N/10 (Xs elapsed)". (5) If changes appear, stop and proceed. (6) After 5 min with no changes, use AskUserQuestion: "Codex companion has not produced file changes after 5 minutes. Options: (a) Keep polling 10 more minutes (b) Retry with Codex (c) Fallback to Claude (d) Check if Codex is still running (`ps aux | grep codex`)". Repeat step 6 every 10 min if user chooses (a). Do NOT say "waiting" without actually running the poll loop.' },
